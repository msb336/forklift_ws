...
def placeholder_inputs(batch_size, num_point):
    ...
    distance_pl = tf.placeholder(tf.float32, shape=(batch_size, 3))
    labels_pl = tf.placeholder(tf.float32, shape=(batch_size, 1))
    return pointclouds_pl, distance_pl, labels_pl


def get_model(point_cloud, distance, is_training, bn_decay=None):
    ...
    net = tf_util.dropout(net, keep_prob=0.7, is_training=is_training,
                          scope='dp2')
    net = tf.concat([net,distance], axis=1)
