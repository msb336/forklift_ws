...
class Reward:
    ...
    def __init__(self, object_pose):
        ...
        self.convertToEuler(quaternion)
        self.calculateTransformToObjectFrame()
    def convertToEuler(self, quaternion):
        self.rotation_ = _euler_from_quaternion(quaternion, 'sxyz')[2]
    def calculateTransformToObjectFrame(self):
        self.transform_matrix_ = np.matmul(np.array(([np.cos(self.rotation_), -np.sin(self.rotation_),0], [np.sin(self.rotation_), np.cos(self.rotation_),0],[0,0,1])), np.array(([1,0,-self.center_[0]], [0,1,-self.center_[1]],[0,0,1])))
    def getOffset(self, vehicle_pose):
        pose_in_object_frame = np.matmul(self.transform_matrix_ , np.array(([vehicle_pose.position.x_val], [vehicle_pose.position.y_val], [1])))
        ...
        if self.distance_ > 0:
            self.angular_offset_ = vehicle_rotation - np.pi - self.rotation_
        else:
            self.angular_offset_ = vehicle_rotation - self.rotation_
